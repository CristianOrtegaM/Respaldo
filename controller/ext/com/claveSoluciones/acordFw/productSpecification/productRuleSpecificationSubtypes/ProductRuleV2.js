Ext.define('AFW_FND_Xjs.controller.ext.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV2', {
    extend: 'Ext.app.Controller',

    stores: ['AFW_FND_Xjs.store.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1'],

    models: ['AFW_FND_Xjs.model.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1'],

    views:  [
             'AFW_FND_Xjs.view.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1PrincipalForm',
             'AFW_FND_Xjs.view.ext.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV2PrincipalWindow',
             'AFW_FND_Xjs.view.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1FormSearch',
             'AFW_FND_Xjs.view.ext.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV2FormInput',
             'AFW_FND_Xjs.view.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1Grid',
             'AFW_FND_Xjs.validation.gen.model.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1Validation'
            ],

    init: function() {
        this.control({
            'productrulev1formsearch button[action=buscar]': {
                click: this.buscar
            },
            'productrulev1grid button[action=confirmarAccion]': {
                click: this.confirmarAccion
            },
            'productrulev2principalwindow button[action=create]': {
                click: this.create
            },
            'productrulev1grid button[action=delete]': {
                click: this.deleteElement
            },
            'productrulev1grid button[action=edit]': {
                click: this.edit
            },
            'productrulev2principalwindow button[action=update]': {
                click: this.update
            },
            'productrulev1grid button[action=mostrarWindows]': {
                click: this.mostrarWindows
            },
            'productrulev2forminput button[action=ruleApplicabilityPrsPanelGridButton]': {
...
//        if(form.isValid() && ruleApplicabilityPrsGrid.count()>0){
        if(form.isValid()){
            ...
            var productRuleV1Record =  form.getRecord();
            if (productRuleV1Record !== undefined 
                && productRuleV1Record !== null 
                && productRuleV1Record .get('specificationIdentifierSpe')!==null 
                && productRuleV1Record .get('specificationIdentifierSpe')!==undefined 
                && new String(productRuleV1Record.get('specificationIdentifierSpe')).indexOf('ProductRuleV1') === -1){
                    btn.setDisabled(false);
                    this.update(btn);
            }else{
                var productRuleV1 = Ext.create('AFW_FND_Xjs.model.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1', {});
				objeto = this.application.getConvertion().convert (objeto, productRuleV1);
                productRuleV1.set(objeto);
                productRuleV1.set({
                ...
                var productRuleV1Validation = Ext.create('AFW_FND_Xjs.validation.gen.model.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1Validation', {});
                var validations = productRuleV1Validation.createValidations (productRuleV1);
                var errors = null;
                    ...
                    callback: function (record, operation) {
                        if (operation.success === true) {
                            ...
                            if (respuesta.valido === true) {
                                ...
                                //Ext.StoreMgr.lookup('AFW_FND_Xjs.store.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1').reload();
                                if(Ext.ComponentQuery.query('#attributeProduct').length >= 0){
                                    Ext.ComponentQuery.query('#attributeProduct')[Ext.ComponentQuery.query('#attributeProduct').length -1].fireEvent('loadTitle', Ext.ComponentQuery.query('#attributeProduct')[Ext.ComponentQuery.query('#attributeProduct').length-1]);
                                    Ext.ComponentQuery.query('#attributeProduct')[Ext.ComponentQuery.query('#attributeProduct').length -1].down('grid').getStore().load();
                                }
                            } else {
                                crearVentana(respuesta.codigo, respuesta.mensaje);
                            }
...
//            }
    ...
    edit: function (btn) {
        ...
        if (seleccion.length > 0) {
            var window = Ext.widget('productrulev2principalwindow');
            window.setTitle('Regla de Producto Nº ' + seleccion[0].get('specificationIdentifierSpe'));
    ...
    update: function(btn) {
        ...
        var objeto = form.getValues(false, true, false);
        var productRuleV1 = form.getRecord();
		objeto = this.application.getConvertion().convert (objeto, productRuleV1);
        productRuleV1.set (objeto);
        productRuleV1.set({
            designerSpe: designerSpe,
    ...
    mostrarWindows: function(btn) {
        ...
        this.application.loadController('AFW_FND_Xjs.controller.ext.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.RuleApplicabilityV1');
        this.application.loadController('AFW_FND_Xjs.controller.gen.com.claveSoluciones.acordFw.roleAndRelationship.PartyRoleV1');
        var ventana = Ext.widget('productrulev2principalwindow');
        
//        var record = btn.up('window').down('form').getForm().getRecord();
//        ventana.show(undefined, function(){
//    		ventana.down('#controlledProductSpecificationPrsGrid').getStore().loadData(record.data,false);
//        });
        try{
    ...
    buscar: function(btn) {
        if(btn.up('form').getForm().isValid()){
            btn.setDisabled(true);
            var store = Ext.StoreMgr.lookup('AFW_FND_Xjs.store.gen.com.claveSoluciones.acordFw.productSpecification.productRuleSpecificationSubtypes.ProductRuleV1');
            store.removeAll ();
            store.filters.clear();
            delete store.getProxy().extraParams['filters'];
            ...
            invalidFields = btn.up('viewport').down('productrulev1formsearch').query("field{isValid()==false}");
            var msg = "Formulario no válido. Complete los campos requeridos:<br />";
            for (var i = 0; i<invalidFields.length; i++){
                msg += '<b>- ' + invalidFields[i].fieldLabel + '</b>. ';
                    for(var j = 0; j<invalidFields[i].getErrors().length; j++){
