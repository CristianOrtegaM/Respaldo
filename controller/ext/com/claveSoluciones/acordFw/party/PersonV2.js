Ext.define('AFW_FND_Xjs.controller.ext.com.claveSoluciones.acordFw.party.PersonV2', {
    extend: 'AFW_FND_Xjs.controller.gen.com.claveSoluciones.acordFw.party.PersonV1',
    ...
    views:  [
             ...
             'AFW_FND_Xjs.view.ext.com.claveSoluciones.acordFw.party.PersonV2PrincipalWindow',
             'AFW_FND_Xjs.view.gen.com.claveSoluciones.acordFw.party.PersonV1FormSearch',
             'AFW_FND_Xjs.view.ext.com.claveSoluciones.acordFw.party.PersonV2FormInput',
    ...
    create: function(btn) {
        ...
        if(form.isValid()
            ...
            if (personV1Record !== undefined 
            	...
            	if(objeto.givenNamePen!="" && objeto.givenNamePen!=null && objeto.surnamePen!="" && objeto.surnamePen!=null){
            		var rec = Ext.create('AFW_FND_Xjs.model.com.claveSoluciones.acordFw.party.partyName.PersonName',{
            			prefixTitleCodePen: objeto.prefixTitleCodePen,
            			givenNamePen: objeto.givenNamePen,
            			middleNamePen: objeto.middleNamePen,
            			surnamePen: objeto.surnamePen,
            			fullNamePan: objeto.givenNamePen+ " " + objeto.middleNamePen + " " +objeto.surnamePen,
            			suffixPen: objeto.suffixPen,
            			languageExternalCodePan: 'SPA',
            			usageCodeOrn: 'LegalName',
            			effectivePeriodStartDateTimePan: new Date(),
            			descriptionPan: objeto.givenNamePen+ " " + objeto.middleNamePen + " " +objeto.surnamePen,
    ...
    edit: function (btn) {
        ...
        if (seleccion.length > 0) {
            var window = Ext.widget('personv2principalwindow');
            window.setTitle('Persona Nº ' + seleccion[0].get('partyIdentifierPar'));
            window.down('form').getForm().loadRecord(seleccion[0]);
            window.show();
            if(seleccion[0].get('namePer')[0]){
        		window.down('combo[name="prefixTitleCodePen"]').setValue(seleccion[0].get('namePer')[0].prefixTitleCodePen);
        		window.down('textfield[name="givenNamePen"]').setValue(seleccion[0].get('namePer')[0].givenNamePen);
        		window.down('textfield[name="middleNamePen"]').setValue(seleccion[0].get('namePer')[0].middleNamePen);
        		window.down('textfield[name="surnamePen"]').setValue(seleccion[0].get('namePer')[0].surnamePen);
        		window.down('textfield[name="suffixPen"]').setValue(seleccion[0].get('namePer')[0].suffixPen);        		
            }
            btn.setDisabled(false);
        } else {
    ...
    update: function(btn) {
		...
		objeto = this.application.getConvertion().convert (objeto, personV1);
        personV1.set (objeto);
        if(person.get('namePer')!=null && person.get('namePer')[0]!=null){
    		var rec = Ext.create('AFW_FND_Xjs.model.com.claveSoluciones.acordFw.party.partyName.PersonName', person.get('namePer')[0]);
    		rec.set({
    			prefixTitleCodePen: objeto.prefixTitleCodePen,
    			givenNamePen: objeto.givenNamePen,
    			middleNamePen: objeto.middleNamePen,
    ...
    mostrarWindows: function(btn) {
        btn.setDisabled(true);
        this.application.loadController('AFW_FND_Xjs.controller.gen.com.claveSoluciones.acordFw.party.partyName.PersonNameV1');
        var ventana = Ext.widget('personv2principalwindow');
        var personV1 = Ext.create('AFW_FND_Xjs.model.gen.com.claveSoluciones.acordFw.party.PersonV1', {
    ...
    buscar: function(btn) {
        if(btn.up('form').getForm().isValid()){
            ...
            if (paramValues.genderCodePer != "" && paramValues.genderCodePer != null) {
                var genderCodePer = Ext.create ('AFW_FND_Xjs.model.util.Filtro', {
                    nombreCampo: 'genderCodePer',
                    valor: paramValues.genderCodePer,
                    tipoValor: 'enum',
            ...
            if (paramValues.ethnicityCodePer != "" && paramValues.ethnicityCodePer != null) {
                var ethnicityCodePer = Ext.create ('AFW_FND_Xjs.model.util.Filtro', {
                    nombreCampo: 'ethnicityCodePer',
                    valor: paramValues.ethnicityCodePer,
                    tipoValor: 'enum',
            ...
            if (paramValues.bloodTypeCodePer != "" && paramValues.bloodTypeCodePer != null) {
                var bloodTypeCodePer = Ext.create ('AFW_FND_Xjs.model.util.Filtro', {
                    nombreCampo: 'bloodTypeCodePer',
                    valor: paramValues.bloodTypeCodePer,
                    tipoValor: 'enum',
            ...
            if (paramValues.maritalStatusCodePer != "" && paramValues.maritalStatusCodePer != null) {
                var maritalStatusCodePer = Ext.create ('AFW_FND_Xjs.model.util.Filtro', {
                    nombreCampo: 'maritalStatusCodePer',
                    valor: paramValues.maritalStatusCodePer,
                    tipoValor: 'enum',
            ...
            if (paramValues.primaryLanguageExternalCodePer != "" && paramValues.primaryLanguageExternalCodePer != null) {
                var primaryLanguageExternalCodePer = Ext.create ('AFW_FND_Xjs.model.util.Filtro', {
                    nombreCampo: 'primaryLanguageExternalCodePer',
                    valor: paramValues.primaryLanguageExternalCodePer+'%',
                    operacion: 'like',
                    tipoValor: 'string'
                });
